...
### 기본 기능
- 일본어 단어/문장 입력 → 형태소별 JLPT 레벨 판정 (N1~N5 / 미등재)
- 각 단어에 한국어 뜻 표시
- **한국어 번역** — 입력 전문을 Google 번역으로 자동 번역하여 결과 하단에 표시
- 다크/라이트 모드 전환
- 형태소 병합 기능 (예: `食べ` + `る` → `食べる`)
- 수정 내역 자동 저장 (localStorage)
- 결과 내보내기 (JSON / CSV / TXT)
- ☁ 편집 데이터 동기화 (GitHub 연동)
...
| 기능 | 설명 |
|------|------|
| 토큰 필드 편집 | 읽기 · 레벨 · 품사 · 한자표기 · 사전형 직접 수정 |
| 한국어 뜻 편집 | 분석 결과의 뜻을 클릭하여 인라인 편집 |
| 형태소 병합/해제 | + / ✕ 버튼으로 인접 토큰 병합 |
| ✏ 사전 수정 | 전체 JLPT 사전 검색 · 편집 (탭: 전체/수정된/커스텀 단어) |
| 📖 사전 CSV 다운로드 | 전체 사전을 CSV로 다운로드. 한국어 뜻이 없는 단어는 Google 번역으로 자동 보완 |
...
| 🔒 / 🔓 | 관리자 모드 진입/종료 | 항상 |
| 📋 | 레벨 분류 기준 CSV 업로드 | 항상 |
| ☁ | 편집 데이터 GitHub 동기화 | 항상 |
| 📖 | 전체 사전 CSV 다운로드 (번역 포함) | 관리자 모드 |
| ✏ | 사전 수정 모달 | 관리자 모드 |
...
└── dist/
    ...
    ├── tangoya_template.html   빌드용 템플릿 (~154 KB)
...
- **형태소 분석**: Kuromoji.js v0.1.2 (IPAdic 사전, Web Worker 비동기)
- **JLPT 데이터**: N1~N5 총 13,680개 항목 (6,823개 고유 단어) + 한국어 뜻 내장
- **한국어 번역**: Google Translate 비공식 API (`client=gtx`, API 키 불필요)
